...
MenuHelpers.rectSwell = (w, dur, timing, delay = 0)->
  setTimeout((->
    this.animate( { width: w}, dur, timing)).bind(this)
  , delay)
...
class window.Menu
  constructor: (parent, items, options = {})->
    ...
    @w = items.reduce((max, item)->
      cur = item.label.length
      r = if (cur > max) then cur else max
    , 0) * 10 + 85
    @h = items.length * 32
  ...
  open: (x,y)->
    ...
    @items.forEach((v, i)->
      ...
      r = @s.rect(
        this.xoff(i) - @options.nodes.radius,
        this.yoff(i) - @options.nodes.radius,
        0, 
        2 * @options.nodes.radius,
        @options.nodes.radius,
        @options.nodes.radius
      )
      r.attr('fill', 'white')
      MenuHelpers.rectSwell.bind(r)(
        @w - this.xoff(i),
        @options.animation.time
        @options.animation.function,
